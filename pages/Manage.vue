<template>
  <div class="div-receive-row">
    ...
    <div class="add-button">
      <button class="add-button" @click="$router.push(`/add_inform`)">
        เพิ่มข้อมูลเจ้าหน้าที่ประสานงาน
      </button>
    </div>
        ...
        <tr v-for="(item, idex) in response" :key="idex">
          ...
          <td>
            <div class="button-abot">
              <button
                ...
                @click="
                  $router.push({
                    ...
                    path: '/edit_recipipent',
                    query: { id: item.recipient_id, idex: idex },
                  })
                "
              >
                แก้ไข
              ...
              <button
                class="button-edit-2"
                @click="deleteinfrmation(item.recipient_id)"
              >
                ลบข้อมูล
              </button>
            </div>
...
export default {
  ...
  methods: {
    ...
    GET00_u() {
      axios
        .get(
          `${URL_GET_RECIPIENT_ALL}/?staus=ทั้งหมด&page=${this.page}&limit=10`
        )
        .then((response) => {
    ...
    GETset_length() {
      axios
        .get(
          `${URL_GET_RECIPIENT_ALL}/?staus=ทั้งหมด&user_id=${localStorage.recipient_id}`
        )
        .then((response) => {
          this.response = response.data.results;
        });
