        ...
        <tr v-for="(item, idex) in response" :key="idex">
          ...
          <td>
            <div class="button-abot">
              ...
              <button
                class="button-edit-2"
                @click="deleteinfrmation(item.user_id)"
              >
                ลบข้อมูล
              </button>
            </div>
...
export default {
  ...
  methods: {
    ...
    GET00_u() {
      axios
        .get(`${URL_GET_USERS}/?staus=ทั้งหมด&page=${this.page}&limit=10`)
        .then((response) => {
    ...
    GETset_length() {
      axios
        .get(`${URL_GET_USERS}/?staus=ทั้งหมด&user_id=${localStorage.users_id}`)
        .then((response) => {
          this.response = response.data.results;
        });
    },
