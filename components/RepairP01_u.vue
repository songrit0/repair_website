<template>
	<div class="div-receive-row">
		...
		<div class="div-item01" v-if="response.length >= 1">
			<div class="col-3">
...
export default {
	...
	methods: {
		...
		GETdata01() {
			axios.get(
				// `${URL_GET_REQ}/?search=${this.searching}&staus=&user_id=${localStorage.users_id}&page=${this.page}&limit=5`
				`${URL_GET_REQ}/?search=${this.searching}&staus=รอตอบรับ&user_id=${localStorage.users_id}&page=${this.page}&limit=5`
				).then(response => {
