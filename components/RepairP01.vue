<template>
  <div class="div-receive-row">
      ...
      <div class="item" v-for="(item, index) in response" :key="index">
        <div class="item1 col-3">
          <div class="div-img">
            ...
            <img
              v-if="item.equipment == 'คอมพิวเตอร์แม่ข่าย'"
              src="../img/computer_FILL0_wght400_GRAD0_opsz48.svg"
              alt="computer"
              width="40px"
            />
            <img
              v-if="item.equipment == 'คอมพิวเตอร์ตั้งโต๊ะ(pc)'"
              src="../img/computer_FILL0_wght400_GRAD0_opsz48.svg"
              alt="computer"
              width="40px"
            />
            <img
              v-if="item.equipment == 'คอมพิวเตอร์พกพา(NoteBook)'"
              src="../img/computer_FILL0_wght400_GRAD0_opsz48.svg"
              alt="computer"
              width="40px"
            />
            <img
              v-if="item.equipment == 'เครื่องสแกนเนอร์(Scnner)'"
              src="../img/print_FILL0_wght400_GRAD0_opsz48.svg"
              alt="print"
              width="40px"
            />
            <img
              v-if="item.equipment == 'ครื่องพิมพ์(printer)'"
              src="../img/print_FILL0_wght400_GRAD0_opsz48.svg"
              alt="print"
              width="40px"
            />
            <img
              v-if="item.equipment == 'แป้นพิมพ์(KeyBoard)'"
              src="../img/keyboard_FILL0_wght400_GRAD0_opsz48.svg"
              alt="keyboard"
              width="40px"
            />
            <img
              v-if="item.equipment == 'เม้าส์usb(mouseusb)'"
              src="../img/mouse_FILL0_wght400_GRAD0_opsz48.svg"
              alt="mouse"
              width="40px"
            />
            <img
              v-if="item.equipment == 'อินเทอร์เน็ต(Internet)'"
              src="../img/signal_wifi_statusbar_not_connected_FILL0_wght400_GRAD0_opsz48.svg"
              alt="internet"
              width="40px"
            />
            <img
              v-if="item.equipment === 'อื่นๆ'"
              src="../img/other_admission_FILL0_wght400_GRAD0_opsz48.svg"
              alt="other"
              width="40px"
            />
          </div>
        ...
        >
          ...
          <button
            class="btn-info btn"
            @click="Showformitem(true, item.id_repair_i)"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
          ...
          <button
            ...
            @click="button(item.id_repair_i, 'ยกเลิก')"
          >
            ยกเลิก
          </button>
          <button
            class="btn-success btn"
            @click="button(item.id_repair_i, 'กำลังดำเนินการ')"
          >
            ตอบรับ
          </button>
        </div>
    ...
    <div class="div-staus">
      <button
        type="button"
        class="btn btn-warning col-12 button-re"
        @click="clickRE()"
      >
        <li>โหลดข้อมูล</li>
        <svg
...
export default {
  ...
  methods: {
              ...
              .then((response) => {
                ...
                if (response.data.status === "ok") {
                  Swal.fire({
                    ...
                    axios
                      .get(
                        `${URL_GET_REQ}/?staus=รอตอบรับ&page=${this.page}&limit=10`
                      )
                      .then((response) => {
                        this.response = response.data.results;
                      });
                    axios
                      .get(`${URL_GET_ALL_REQ}/?staus=รอตอบรับ`)
                      .then((response) => {
                        this.get_lengthdata.process01 =
                          response.data.lengthdata;
                        var sum = response.data.lengthdata / 10;
                        this.set_length = Math.ceil(sum);
                      });
                    axios
                      .get(`${URL_GET_ALL_REQ}/?staus=กำลังดำเนินการ`)
                      .then((response) => {
                        this.get_lengthdata.process02 =
                          response.data.lengthdata;
                      });
                  });
                }
    ...
    GETdata01() {
      axios
        .get(`${URL_GET_REQ}/?staus=รอตอบรับ&page=${this.page}&limit=10`)
        .then((response) => {
          this.response = response.data.results;
        });
