    ...
    <div class="div-item2" v-if="response.length >= 1">
      <table>
        ...
        <tr v-for="(item, idex) in response" :key="idex">
          ...
          <td>
            <button @click="Showformitem(true, item.id_repair_i)">
              ข้อมูลเพิ่มเติม
            </button>
          </td>
        </tr>
...
export default {
  ...
  methods: {
        ...
        .then((response) => {
          ...
          if (response.data.status === "ok") {
            Swal.fire({
              ...
              axios
                .get(
                  `${URL_GET_REQ}/?staus="ซ่อมเสร็จ"&page=${this.page}&limit=10`
                )
                .then((response) => {
                  this.response = response.data.results;
                });
              axios
                .get(`${URL_GET_ALL_REQ}/?staus=ซ่อมเสร็จ`)
                .then((response) => {
    ...
    GETdata03() {
      axios
        .get(`${URL_GET_REQ}/?staus=ซ่อมเสร็จ&page=${this.page}&limit=10`)
        .then((response) => {
          this.response = response.data.results;
